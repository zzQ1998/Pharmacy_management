...
//Tooltip
$(function() {
    $(".tooltip-trigger").each(function(b) {
        if (this.title) {
            var c = this.title;
            var a = 5;
...
$('.collect').click(function() {
    ...
    //点赞操作
    if (_this.attr('uid') && _this.hasClass('collect-no')) { //如果uid存在(已登录)，用户还没有点击收藏
        // 获取用户id
        ...
        $.ajax({
            ...
            dataType: 'html',
            url: 'collect',
            data: 'uid=' + uid + '&artid=' + artid + '&act=add', //传到url的参数
            cache: false,
            success: function() { //成功提示
        ...
        return false;
    } else if (_this.attr('uid') && _this.hasClass('collect-yes')) { //取消点赞
        var uid = Number(_this.attr('uid'));
        $.ajax({
            type: 'POST',
            dataType: 'html',
...
$('.buysaledl').click(function() {
    ...
    $.ajax({
        ...
        success: function(check) {
            login_in = check.status;
            if (login_in != 0) {
                $.ajax({
                    type: 'POST',
                    dataType: 'json',
                    url: tin.ajax_url,
                    data: 'action=popsaledl&sid=' + sid + '&pid=' + pid + '&uid=' + uid,
                    cache: false,
                    success: function(msg) {
                        ...
                        $('.dl-price').html(msg.price);
                        $('.all-credits').html(msg.credit);
                        enough = Number(msg.enough);
                        if (enough == 0) {
                            $('.saledl-msg').html('抱歉，你当前的积分不足以支付该资源！ <a href="http://www.zhiyanblog.com/how-to-earn-credits.html" title="赚取积分" target="_blank">如何赚取积分?</a>');
                        } else {
                            $('.saledl-msg').html('');
...
function Confirm_to_buy() {
    ...
    $.ajax({
        ...
        dataType: 'json',
        url: tin.ajax_url,
        data: 'action=confirmbuy&sid=' + sid + '&pid=' + pid + '&uid=' + uid,
        cache: false,
        success: function(msg) {
...
$('button#unsubscribe').click(function() {
    ...
    if (email == '' || (!email.match('^([a-zA-Z0-9_-])+((\.)?([a-zA-Z0-9_-])+)+@([a-zA-Z0-9_-])+(\.([a-zA-Z0-9_-])+)+$'))) {
        ...
        $.ajax({
            ...
            dataType: 'json',
            url: tin.ajax_url,
            data: 'action=unsubscribe&email=' + email,
            cache: false,
